...
public class CardScript : MonoBehaviour
{
    ...
    private string SEPARATOR = "|"; //S�parateur des param�tres de la description
    private string PLACEHOLDER = "param�tre introuvable";
    public Vector3 Target_Position; //Une position dont on doit se rapprocher
    public bool Must_Reach_Target=true; //Doit se d�placer vers sa position cible
    public float Max_Speed = 2; //La vitesse par frame dont on se d�place
    ...
    //S�par�e du reste pour pouvoir �tre appel�e ind�pendanment par le Loading_Effect
    ...
    //Prend le texte "brute" de la description, puis rempli les param�tres en mettant leur valeur
    string Fill_Description(string description)
    {
        //On va d'abord s�parer les strings en s�gment
        string[] segments = description.Split(SEPARATOR);

        //On va regarder si le premier mot est un param�tre en regardant si la chaine commence par |
        //Si oui, on va prendre les bouts de string avec un index paire (0, 2, etc.) car ce sont les param�tres
        ...
        //On remplace toutes les cl�s de param�tres par leurs valeurs
